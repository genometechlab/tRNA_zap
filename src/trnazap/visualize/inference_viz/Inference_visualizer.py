...
class ZAPInferenceVisualizer:
    ...
    def __init__(
        ...
        config: Optional[Union["ModelConfig", str, Dict]] = None,
        signal_scale: float = 1000.0,
        device: Optional[torch.device] = None,
        color_map: Optional[Dict[int, str]] = None,
        class_labels: Optional[Dict[int, str]] = None,
        rasterize_signal: bool = False,
        signal_linewidth: float = 0.6,
    ) -> None:
        ...
        if config is not None:
            self.config = ModelConfig.load_config(cfg=config)
    ...
    def _create_visualization(
        ...
        # Format
        self._format_figure(ax, read_result.read_id, predicted_cls, len(signal_scaled))
        ...
        if predicted_cls is not None:
            if hasattr(self, 'config') and isinstance(self.config, ModelConfig):
                ...
                if label_names is not None:
                    predicted_cls = label_names[predicted_cls]
            title += f", Prediction: {predicted_cls}"
