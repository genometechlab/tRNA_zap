...
class TransformerZAM_multitask_wfrag(nn.Module):
    def __init__(
        self,
        input_size: int = 64,
        hidden_size: int = 256,
        ...
        warnings.warn(
            "TransformerZAM_multitask_wfrag is deprecated and will be removed in a future version. "
    ...
    def forward(self, signal: torch.Tensor, length: torch.Tensor):
        ...
        frag_representation = torch.cat([cls_representation, 
                                         token_representation.mean(dim=1)], 
                                        dim=1)  # Shape: [B, 2D]

        frag_logits = self.frag_classifier(frag_representation)
        seq_logits = self.classifier(cls_representation)
        token_logits = self.token_classifier(token_representation)

        return {
            "fragmentation": frag_logits,
