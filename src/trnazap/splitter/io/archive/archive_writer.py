# src/trnazap/splitter/io/archive/archive_writer.py
"""Writer for ZIR archive format."""
...
class ZIRWriter:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if self.file:
            # Go back and update record count in header
    ...
    def _write_header(self):
        ...
        # Pad to HEADER_SIZE
        current_size = len(MAGIC_BYTES) + 4 + 4 + 4 + len(metadata_bytes)
        padding = HEADER_SIZE - current_size
        if padding > 0:
            self.file.write(b'\x00' * padding)
        elif padding < 0:
            raise ValueError(f"Metadata too large: {len(metadata_bytes)} bytes")
        
    def add_result(self, result: 'ReadResult'):
        """
        Add a single result to the archive.
        
        Args:
            result: ReadResult to add
        """
        if not self.file:
        ...
        # Write read ID
        read_id_bytes = result.read_id.encode('utf-8')
        buffer.extend(struct.pack('<H', len(read_id_bytes)))  # 2 bytes for length
        buffer.extend(read_id_bytes)
        
        # Write basic metadata
        buffer.extend(struct.pack('<i', result.num_chunks))  # 4 bytes
        buffer.extend(struct.pack('<i', result.chunk_size))  # 4 bytes
        
        # Write classification logits if present
        if result.classification_logits is not None:
            buffer.extend(struct.pack('<B', 1))  # Has classification flag
            cls_array = result.classification_logits.astype('float32')
        ...
        if result.seq2seq_logits is not None:
            buffer.extend(struct.pack('<B', 1))  # Has seq2seq flag
            seq_array = result.seq2seq_logits.astype('float32')
        ...
        # Compress the buffer
        compressed = self.compressor.compress(bytes(buffer))
