...
def fragment_align(sub_sequence, ref_sequence, min_len=15):
    """
    Perform edit distance based sub-alignments of query to reference.
    
    These alignments are performed to identify the optimal local 
    alignment when the full alignment includes too high of a 
    deletion proportion. By enforcing a minimum length of aligned 
    sequence many of the shorter sequences are removed from 
    consideration.
...
def align_read(
    ...
    if check_fragment(a.cigar, len(ref_sequence)):
        a.cigar, edit_dist, ref_start, ref_stop, query_start, query_stop = fragment_align(sub_sequence, ref_sequence, min_len=15)
        a.set_tag("FRG",1)
